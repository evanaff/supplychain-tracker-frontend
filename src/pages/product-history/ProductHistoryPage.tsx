...
export default function ProductHistoryPage() {
    ...
    const verifyMutation = useMutation({
        mutationFn: () => productLotsApi.verify(id!).then((r) => r.data.data),
    ...
    return (
        <>
                ...
                <meta
                    ...
                    content={
                        pl
                            ? `Product history for lot ${pl.lotNumber} - ${pl.product?.varietyName ?? pl.product.gtin}. View full supply chain journey and blockchain verification.`
                ...
                <div className="container max-w-3xl mx-auto px-4 py-8 space-y-6 animate-slide-up">
                    {isLoading ? (
                        ...
                        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-6 flex gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-destructive">Product lot not found</p>
                                <p className="text-sm text-muted-foreground mt-1">
                                    {error?.message || "This product lot ID does not exist or has been removed."}
                                </p>
                            </div>
                        </div>
                    ) : (
                        <>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-0 sm:gap-6">
                                        ...
                                        <div className="flex-1 space-y-4 p-6 sm:p-0">
                                            ...
                                            <div className="flex flex-col gap-4 text-sm bg-muted/30 p-4 rounded-lg">
                                                ...
                                                <p className="font-medium">{pl.product?.gtin ?? pl.product.gtin ?? '-'}</p>
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <p className="text-muted-foreground text-xs mb-1">Lot Number</p>
                                                    <p className="font-medium">{pl.lotNumber}</p>
                                                </div>
                                                <div>
                                                    <p className="text-muted-foreground text-xs mb-1">Quantity</p>
                                                    <p className="font-medium">{pl.quantity.toLocaleString()} {pl.product?.unitOfMeasure ?? 'units'}</p>
                                                </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4 pt-2 animate-slide-up">
                                            ...
                                            {verificationResult.invalidEvents.length === 0 &&
                                                ...
                                                <div className="flex items-center gap-2.5 rounded-lg bg-emerald-50 border border-emerald-200 p-3 text-sm text-emerald-700">
                                                    ...
                                                    All {verificationResult.totalEvents} events verified on the blockchain
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-2.5 rounded-lg bg-red-50 border border-red-200 p-3 text-sm text-red-700">
                                                    <ShieldX className="h-4 w-4 shrink-0" />
                                                    Some events are invalid or missing. Please report this verification result.
                                                </div>
                                            )}
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <ProductEventTimeline events={events} verificationResult={verificationResult} hideChainStatus={true} />
                                </CardContent>
                            </Card>
                        </>
                    )}
